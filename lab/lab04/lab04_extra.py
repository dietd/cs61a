...
def merge(lst1, lst2):
    ...
    while i < len(lst1) and k < len(lst2):
        if lst1[i] < lst2[k]:
            ...
            k += 1
    if i >= len(lst1):
        return new + lst2[k:len(lst2)]
    if k >= len(lst2):
        return new + lst1[i:len(lst1)]
...
def put_piece(board, max_rows, column, player):
    ...
    i = max_rows
    while i > 0:
        if get_piece(board, i-1, column) != '-':
            new_board[i] = replace_elem(new_board[i], column, player)
            return (i, new_board)
        i -= 1
    return   
