...
def is_swap(player_score, opponent_score):
    ...
    opponent = opponent_score % 100
    player_difference = (player // 10) - (player % 10)
    opponent_difference = (opponent // 10) - (opponent % 10)
    if abs(player_difference) == abs(opponent_difference):
        return True
    else:
        return False
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if current_score-previous_score > previous_high:
            print(abs(current_score-previous_score), "point(s)! That's the biggest gain yet for Player", who)
            high = current_score-previous_score
        return announce_highest(who, high, current_score)
    return say
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    1
    """
    num_rolls, high_score, n = 1, 0, 0

    averaged = make_averaged(roll_dice, num_samples)
    while num_rolls <= 10:
        avg = averaged(num_rolls, dice)
        if avg > high_score:
            high_score = avg
            n = num_rolls
        num_rolls += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    future_score = free_bacon(opponent_score) + score
    if is_swap(future_score, opponent_score):
        if future_score < opponent_score:
            return 0
        elif future_score > opponent_score:
            return num_rolls
        else:
            return num_rolls
    else:
