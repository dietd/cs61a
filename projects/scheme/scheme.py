...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        if self.parent is None and self.bindings.get(symbol) is None:
            raise SchemeError('unknown identifier: {0}'.format(symbol))
        elif self.bindings.get(symbol) is None:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            ...
            if (formals is nil and vals) or (vals is nil and formals):
                raise SchemeError()
        return newframe
...
def do_and_form(expressions, env):
    ...
    while expressions.second is not nil:
        ...
        if (not current and current != 0) or current is False:
            return current
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbols, values = Pair(current.first, symbols), Pair(scheme_eval(current.second.first,env), values)
...
def do_define_macro(expressions, env):
    ...
    if self_evaluating(expressions.first.first):
        raise SchemeError()
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        """Rebinds formals"""
        symbol = self.formals
        while args is not nil and symbol is not nil:
            newframe.define(symbol.first, args.first)
            args, symbol = args.second, symbol.second
        return newframe
