...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    centroid_list = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
    return group_by_first(centroid_list)
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    longitude_list = [restaurant_location(c)[0] for c in cluster]
    latitude_list = [restaurant_location(c)[1] for c in cluster]

    return [mean(longitude_list), mean(latitude_list)]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(group[i]) for i in range(k)]
        # END Question 6
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    rated = [[restaurant_name(r), user_rating(user, restaurant_name(r))] for r in reviewed]
    not_rated = [[restaurant_name(r), predictor(r)] for r in restaurants if r not in reviewed]
    all_ratings = {r[0]: r[1] for r in rated + not_rated}
    return all_ratings
