    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) and not insect.is_container:
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant) and not self.ant.is_container:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.backwards and not self.scared:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while i < self.min_range:
            ...
            i += 1
        while place is not hive and i < self.max_range:
            if place.bees:
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return self.contained_ant is None or other.is_container
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_queen:
            ...
            while place:
                if place.ant:
                    if (isinstance(place.ant, BodyguardAnt) or isinstance(place.ant, TankAnt)) and place.ant.contained_ant:
                        if place.ant.contained_ant not in self.buffed_ants:
                            place.ant.contained_ant.damage *= 2
                            self.buffed_ants += [place.ant.contained_ant]
...
def apply_effect(effect, bee, duration):
    ...
    def helper(colony):
        ...
        if duration > 0:
            duration -= 1
            return effect(act, bee)(colony)
        else:
            bee.backwards, bee.scared = False, True
            return act(colony)
