...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
    	...
    	return tree(label(t), [replace_leaf(r, old, new) for r in branches(t)])
...
def totals_tree(m):
    ...
    if is_mobile(m):
        return tree(total_weight(m), tree(totals_tree(left(m))) + tree(totals_tree(right(m))))
...
def make_counter():
    ...
    def helper(newstr):
        nonlocal lst
        lst += newstr
        return len([ _ for _ in lst if _ == newstr])
    return helper
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
        nonlocal password, balance, attempts
        if len(attempts) >= 3:
            return "Your account is locked. Attempts: " + str(attempts)
        if p != password or p not in password:
            attempts = attempts + [p]
            return "Incorrect password"
        elif p == password or p in password:
            if amount <= balance:
                balance -= amount
            else:
...
def generate_paths(t, x):
    ...
    """
    "*** YOUR CODE HERE ***"
    stack = [(label(t), [label(t)], t)]
    while stack:
        (v, path, tree) = stack.pop()
        for n in branches(tree):
            if label(n) == x:
                yield path + [label(n)]
            stack.append((label(n), path + [label(n)], n))
